...
def terminator_variants(D):
    ...
    for sign, term in ((+1, lon_sub + 90.0), (-1, lon_sub - 90.0)):
        ...
        for j in range(len(ts_lat)):
            ...
            o = np.argsort(pp[ok]); P_, U_, S_ = pp[ok][o], uu[ok][o], ss[ok][o]
            w = A * dphi[j]
            e = np.trapezoid(U_ * S_, P_) / G; m = np.trapezoid(U_, P_) / G
            sm = np.trapezoid(S_, P_) / G; mm = np.trapezoid(np.ones_like(P_), P_) / G
            out['col'] += sign * w * (e - (sm / mm) * m)          # per-column barotropic removal
            Eb += w * e; Mb += w * m; SMb += w * sm; MMb += w * mm
        out['bnd'] += sign * (Eb - (SMb / MMb) * Mb)               # per-terminator reference
    area = 2.0 * np.pi * A ** 2
    off = np.abs(((ts_lon - lon_sub + 180.0) % 360.0) - 180.0); night = off > 90.0
    rad = er.area_mean(np.nanmean((D['olr'] - D['asr'])[:, night], axis=-1), cw)
    return out['col'] / area, out['bnd'] / area, rad
