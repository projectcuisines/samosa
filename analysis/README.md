...
- Section 4 (the extension sequences): `figures/allcases/fig_interpolation_temp_1d.py` prints every number, and `figures/allcases/fit_anisotropy.py` the leave-one-out errors; `figures/allcases/check_table4.py` checks the protocol's Table 4 against its Sobol draws. The universal-kriging paragraph (Figure 19) takes every number from `figures/allcases/fig_interpolation_temp_uk.py`.
