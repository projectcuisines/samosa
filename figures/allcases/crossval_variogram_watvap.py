...
#
# The spurious _s = 0.1 scale factor that once sat on four of these arrays was
# removed from the figure scripts in a32c424 but survived here until 2026-09-02.
# It changed no printed result, because a constant factor is a constant offset
# in log space and the variogram sees only differences, but it left LFRic on a
# different scale from the rest and made the arrays unusable outside this
# script. kg m^-2 is the common unit.
...
# ExoColumn, cases 1, 4, 8, 9, 10, 11, 14, 15 (kg m^-2).
...
climate_models = [
    ...
    ( 'ExoColumn',   'std',    exocolumn_pres1, exocolumn_flux1, np.log(exocolumn),    None, None, None ),
]
