...
Ts_plasim1  = np.average( f_exoplasim1.variables[ 'clt' ], axis=0 )
Ts_plasim4  = np.average( f_exoplasim4.variables[ 'clt' ], axis=0 )
Ts_plasim16 = np.average( f_exoplasim16.variables[ 'clt' ], axis=0 )
...
Ts_exocam1  = np.average( f_exocam1.variables[ 'CLDTOT' ], axis=0 )
Ts_exocam4  = np.average( f_exocam4.variables[ 'CLDTOT' ], axis=0 )
Ts_exocam16 = np.average( f_exocam16.variables[ 'CLDTOT' ], axis=0 )
...
Ts_pcm4 = np.average( f3.variables[ 'liquid_cloud_fraction' ], axis=0 )
...
f_PCM_noOHT = netCDF4.Dataset( '/models/data/samosa/genericpcm/SAMOSA_output_file_Generic_PCM_case-4_OHT_off.nc' )

Ts_pcm_noOHT4  = np.array( f_PCM_noOHT.variables[ 'liquid_cloud_column' ] )
...
Ts_r3d1  = np.array( f_r3d1.variables[ 'pcldt' ] ) * 0.01
Ts_r3d4  = np.array( f_r3d4.variables[ 'pcldt' ] ) * 0.01
Ts_r3d16 = np.array( f_r3d16.variables[ 'pcldt' ] ) * 0.01
...
Ts_lfric4 = np.sum( f6.variables[ 'frozen_fraction' ], axis=0 )
...
im6 = ax_array[0, 4].contourf( lon_pcm, lat_pcm, ZEROVALS, cmap=cm, vmin=contourmin, vmax=contourmax, levels=np.linspace(contourmin,contourmax,numcontours), extend='both' )
ax_array[0, 4].set_title( 'LFRic', fontsize=15 )
ax_array[0, 4].set_xticks( [ ], labels=[] )
ax_array[0, 4].set_yticks( [ ], labels=[] )

im6 = ax_array[1, 4].contourf( lon_lfric, lat_lfric, Ts_lfric4, cmap=cm, vmin=contourmin, vmax=contourmax, levels=np.linspace(contourmin,contourmax,numcontours), extend='both' )
...
ZEROVALS = np.zeros( ( np.size( lat_pcm ), np.size( lon_pcm ) ) )
im3 = ax_array[0, 5].contourf( lon_pcm, lat_pcm, ZEROVALS, cmap=cm, vmin=contourmin, vmax=contourmax, levels=np.linspace(contourmin,contourmax,numcontours), extend='both' )
ax_array[0, 5].set_title( 'Gen. PCM', fontsize=15 )
...
ZEROVALS = np.zeros( ( np.size( lat_pcm ), np.size( lon_pcm ) ) )
im3 = ax_array[2, 5].contourf( lon_pcm, lat_pcm, ZEROVALS, cmap=cm, vmin=contourmin, vmax=contourmax, levels=np.linspace(contourmin,contourmax,numcontours), extend='both' )
...
ax_array[0, 6].set_title( 'Gen. PCM (w/OHT)', fontsize=15 )
ax_array[0, 6].set_xticks( [ ], labels=[] )
ax_array[0, 6].set_yticks( [ ], labels=[] )

im3 = ax_array[1, 6].contourf( lon_pcm, lat_pcm, Ts_pcm4, cmap=cm, vmin=contourmin, vmax=contourmax, levels=np.linspace(contourmin,contourmax,numcontours), extend='both' )
ax_array[1, 6].set_title( '', fontsize=15 )
ax_array[1, 6].set_xlabel( '', fontsize = 10 )
ax_array[1, 6].set_ylabel( '', fontsize = 10 )
...
cb0 = fig.colorbar( im1, ax=ax_array, extend='both', ticks=np.arange( contourmin, contourmax, 50 ), shrink=0.8, pad=0.01, label='Cloud Fraction (%)' )
