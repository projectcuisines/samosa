...
Ts_plasim1  = np.average( f_exoplasim1.variables[ 'prw' ], axis=0 )
Ts_plasim4  = np.average( f_exoplasim4.variables[ 'prw' ], axis=0 )
Ts_plasim16 = np.average( f_exoplasim16.variables[ 'prw' ], axis=0 )
...
Ts_exocam1  = np.average( f_exocam1.variables[ 'TMQ' ], axis=0 )
Ts_exocam4  = np.average( f_exocam4.variables[ 'TMQ' ], axis=0 )
Ts_exocam16 = np.average( f_exocam16.variables[ 'TMQ' ], axis=0 )
...
#
#Ts_pcm4 = np.array( f3.variables[ 'water_vapor_column' ] )
...
Ts_pcm_noOHT1  = np.array( f_PCM_noOHT1.variables[ 'water_vapor_column' ] )
Ts_pcm_noOHT4  = np.array( f_PCM_noOHT4.variables[ 'water_vapor_column' ] )
Ts_pcm_noOHT16  = np.array( f_PCM_noOHT16.variables[ 'water_vapor_column' ] )
...
Ts_r3d1  = np.array( f_r3d1.variables[ 'qatm' ] )
Ts_r3d4  = np.array( f_r3d4.variables[ 'qatm' ] )
Ts_r3d16 = np.array( f_r3d16.variables[ 'qatm' ] )
...
Ts_lfric1 = f_lfric1.variables[ 'tot_col_m_v' ]
Ts_lfric4 = f_lfric4.variables[ 'tot_col_m_v' ]
...
#cm = mpl.colormaps.get_cmap( 'magma' )
#cm = mpl.colormaps.get_cmap( 'PuBuGn' )
cm = mpl.colormaps.get_cmap( 'YlGnBu' )
#cm = mpl.colormaps.get_cmap( 'BuPu' )
contourmax  = 1.e2
contourmin  = 1.e-3
numcontours = 100
...
im1 = ax_array[0, 0].contourf( lon_exocam, lat_exocam, Ts_exocam1, cmap=cm, levels=np.logspace( np.log10(contourmin), np.log10(contourmax) , numcontours ), extend='both', norm=colors.LogNorm() )
...
im1 = ax_array[1, 0].contourf( lon_exocam, lat_exocam, Ts_exocam4, cmap=cm, levels=np.logspace( np.log10(contourmin), np.log10(contourmax) , numcontours ), extend='both', norm=colors.LogNorm() )
...
im1 = ax_array[2, 0].contourf( lon_exocam, lat_exocam, Ts_exocam16, cmap=cm, levels=np.logspace( np.log10(contourmin), np.log10(contourmax) , numcontours ), extend='both', norm=colors.LogNorm() )
...
im2 = ax_array[0, 1].contourf( lon_plasim1, lat_plasim, Ts_plasim1, cmap=cm, vmin=contourmin, vmax=contourmax, levels=np.logspace( np.log10(contourmin), np.log10(contourmax) , numcontours ), extend='both', norm=colors.LogNorm() )
...
im2 = ax_array[1, 1].contourf( lon_plasim4, lat_plasim, Ts_plasim4, cmap=cm, vmin=contourmin, vmax=contourmax, levels=np.logspace( np.log10(contourmin), np.log10(contourmax) , numcontours ), extend='both', norm=colors.LogNorm() )
...
im2 = ax_array[2, 1].contourf( lon_plasim16, lat_plasim, Ts_plasim16, cmap=cm, vmin=contourmin, vmax=contourmax, levels=np.logspace( np.log10(contourmin), np.log10(contourmax) , numcontours ), extend='both', norm=colors.LogNorm() )
...
im4 = ax_array[0, 2].contourf( lon_r3d, lat_r3d, Ts_r3d1, cmap=cm, vmin=contourmin, vmax=contourmax, levels=np.logspace( np.log10(contourmin), np.log10(contourmax) , numcontours ), extend='both', norm=colors.LogNorm() )
...
im4 = ax_array[1, 2].contourf( lon_r3d, lat_r3d, Ts_r3d4, cmap=cm, vmin=contourmin, vmax=contourmax, levels=np.logspace( np.log10(contourmin), np.log10(contourmax) , numcontours ), extend='both', norm=colors.LogNorm() )
...
im4 = ax_array[2, 2].contourf( lon_r3d, lat_r3d, Ts_r3d16, cmap=cm, vmin=contourmin, vmax=contourmax, levels=np.logspace( np.log10(contourmin), np.log10(contourmax) , numcontours ), extend='both', norm=colors.LogNorm() )
...
im5 = ax_array[0, 3].contourf( lon_plahab, lat_plahab, Ts_plahab1, cmap=cm, vmin=contourmin, vmax=contourmax, levels=np.logspace( np.log10(contourmin), np.log10(contourmax) , numcontours ), extend='both', norm=colors.LogNorm() )
...
im5 = ax_array[1, 3].contourf( lon_plahab, lat_plahab, Ts_plahab4, cmap=cm, vmin=contourmin, vmax=contourmax, levels=np.logspace( np.log10(contourmin), np.log10(contourmax) , numcontours ), extend='both', norm=colors.LogNorm() )
...
im5 = ax_array[2, 3].contourf( lon_plahab, lat_plahab, Ts_plahab16, cmap=cm, vmin=contourmin, vmax=contourmax, levels=np.logspace( np.log10(contourmin), np.log10(contourmax) , numcontours ), extend='both', norm=colors.LogNorm() )
...
im6 = ax_array[1, 4].contourf( lon_lfric, lat_lfric, Ts_lfric4, cmap=cm, vmin=contourmin, vmax=contourmax, levels=np.logspace( np.log10(contourmin), np.log10(contourmax) , numcontours ), extend='both', norm=colors.LogNorm() )
...
im6 = ax_array[2, 4].contourf( lon_pcm, lat_pcm, ZEROVALS, cmap=cm, vmin=contourmin, vmax=contourmax, levels=np.logspace( np.log10(contourmin), np.log10(contourmax) , numcontours ), extend='both', norm=colors.LogNorm() )
...
ZEROVALS = np.zeros( ( np.size( lat_pcm ), np.size( lon_pcm ) ) )
im3 = ax_array[0, 6].contourf( lon_pcm, lat_pcm, ZEROVALS, cmap=cm, vmin=contourmin, vmax=contourmax, levels=np.logspace( np.log10(contourmin), np.log10(contourmax) , numcontours ), extend='both', norm=colors.LogNorm() )
...
ZEROVALS = np.zeros( ( np.size( lat_pcm ), np.size( lon_pcm ) ) )
im3 = ax_array[2, 6].contourf( lon_pcm, lat_pcm, ZEROVALS, cmap=cm, vmin=contourmin, vmax=contourmax, levels=np.logspace( np.log10(contourmin), np.log10(contourmax) , numcontours ), extend='both', norm=colors.LogNorm() )
...
# ZEROVAL Panel
#ZEROVALS = np.zeros( ( np.size( lat_pcm ), np.size( lon_pcm ) ) )
#im6 = axd[ 'P6' ].contourf( lon_pcm, lat_pcm, ZEROVALS, cmap=cm, vmin=contourmin, vmax=contourmax, levels=np.logspace( np.log10(contourmin), np.log10(contourmax) , numcontours ), extend='both', norm=colors.LogNorm() )
...
cb0 = fig.colorbar( im1, ax=ax_array, extend='both', norm=colors.LogNorm( vmin=np.log10(contourmin), vmax=np.log10(contourmax) ), ticks=[1e-4, 1e-3, 1e-2, 1e-1, 1e0, 1e1, 1e2, 1e3], shrink=0.8, pad=0.01, label='Water Vapor Column (kg m$^{-2}$)' )
#cb0 = fig.colorbar( im1, ax=ax_array, extend='both', ticks=np.arange( np.log10(contourmin), np.log10(contourmax), 50 ), shrink=0.8, pad=0.01, label='Water Vapor Column (kg m$^{-2}$)' )
#cb0 = fig.colorbar( im1, ax=ax_array, extend='both', ticks=np.arange( contourmin, contourmax + 50, 50 ), shrink=0.8, pad=0.01, label='Cloud Fraction (%)' )
